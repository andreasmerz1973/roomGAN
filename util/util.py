...
def draw_semantic_from_map(semantic_map):

    np_array = semantic_map[0].cpu().detach().numpy() #16x256x256

    blank = np.array(Image.new('RGB', (256,256), 'black'))

    for dim in range(np_array.shape[0]):
        blank[np_array[dim] != 0 ] = colors[dim]
